...
run(function()
    ...
    local function createKeyButton(name, text, position, size)
        ...
        shadow.Size = UDim2.new(1, 4 * Scale.Value, 1, 4 * Scale.Value)
        shadow.Position = UDim2.new(0, -2 * Scale.Value, 0, -2 * Scale.Value)
        shadow.ZIndex = -1
    ...
    local function updateKeyVisual(key, isPressed)
        ...
        local targetSize = isPressed and UDim2.new(0, button:GetAttribute("BaseWidth") * 0.92, 0, button:GetAttribute("BaseHeight") * 0.92) 
                                    or UDim2.new(0, button:GetAttribute("BaseWidth"), 0, button:GetAttribute("BaseHeight"))
        local targetPos = isPressed and UDim2.new(button.Position.X.Scale, button.Position.X.Offset + (button:GetAttribute("BaseWidth") * 0.04), 
                                                 button.Position.Y.Scale, button.Position.Y.Offset + (button:GetAttribute("BaseHeight") * 0.04))
                                    or UDim2.new(button.Position.X.Scale, button.Position.X.Offset, button.Position.Y.Scale, button.Position.Y.Offset)
        
        -- Color inversion on press with accent
        if isPressed then
            ...
            button:FindFirstChildOfClass("UIStroke").Color = Color3.fromHSV(AccentColor.Hue, AccentColor.Sat * 1.2, math.min(AccentColor.Value + 0.2, 1))
            button:FindFirstChildOfClass("UIStroke").Transparency = 0.3
        else
            button.BackgroundColor3 = Color3.fromHSV(BackgroundColor.Hue, BackgroundColor.Sat, BackgroundColor.Value)
            button.TextColor3 = Color3.fromHSV(TextColor.Hue, TextColor.Sat, TextColor.Value)
            button:FindFirstChildOfClass("UIStroke").Color = Color3.fromHSV(AccentColor.Hue, AccentColor.Sat, AccentColor.Value)
            button:FindFirstChildOfClass("UIStroke").Transparency = 0.6
        end
    ...
    local function buildUI()
        ...
        -- Make draggable with better feedback
    ...
    Keystrokes = vape.Categories.Render:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                -- Input detection with improved responsiveness
        ...
        Tooltip = 'Premium keystroke display with smooth animations and customizable styling. Drag to reposition.'
    })
            ...
            if keystrokeFrame then
                for key, button in keyButtons do
                    if not keyStates[key] then
                        ...
                        if gradient then
                            gradient.Color = ColorSequence.new({
                                ColorSequenceKeypoint.new(0, Color3.fromHSV(hue, sat * 0.8, math.min(val + 0.05, 1))),
                                ColorSequenceKeypoint.new(1, Color3.fromHSV(hue, sat, val))
                            })
                        end
                    end
    ...
    AnimationSpeed = Keystrokes:CreateSlider({
        Name = 'Animation Speed',
        Min = 0.01,
        Max = 0.3,
        Default = 0.08,
